...
dom.sheet.addEventListener("touchstart", (e) => {
    ...
    dom.sheet.style.transition = "none"; // Remove transição durante o arrasto
}, { passive: true });
...
dom.sheet.addEventListener("touchmove", (e) => {
    ...
    // Limite elástico simples
    if (newHeight > sheetMetrics.maxH + 50 || newHeight < sheetMetrics.minH - 20) return;
...
dom.sheet.addEventListener("touchend", (e) => {
    dragging = false;
    dom.sheet.style.transition = ""; // Restaura transição CSS

    const endY = e.changedTouches[0].clientY;
    const delta = dragStartY - endY;
    const h = dom.sheet.offsetHeight;
    
    // Definição de swipe baseado em velocidade/distância
    const isSwipeUp = delta > 40;
    const isSwipeDown = delta < -40;

    // Detecta estado atual aproximado
    const currentClass = dom.sheet.classList.contains("sheet-min") ? "min"
                       : dom.sheet.classList.contains("sheet-mid") ? "mid"
                       : "max";

    if (isSwipeUp) {
        if (currentClass === "min") setSheetState("sheet-mid");
        else setSheetState("sheet-max");
    } 
    else if (isSwipeDown) {
        if (currentClass === "max") setSheetState("sheet-mid");
